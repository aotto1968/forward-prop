...
| Erkenntnis | Detail |
|------------|--------|
| W0 = kritische Nichtlinearität | 4–9pp über der linearen Basis, identische Bit-Masse |
...
> (`mlp-bin32-otto-trn-seq.c`) built with `-DKI_BITVOTING`: identity W0
> (gb = input), `splitVN=1` forced, `H = I` width. The binary names below
> are unchanged — the Makefile `bitvote` targets now build the seq.c trainer
> with the flag. Target init is the standard TRN recipe (count →
...
| File | Description |
|------|-------------|
| `otto-score-ifc/mnist/mlp-bin32-otto-trn-seq.c` | Shared trainer — Bit-Voting via `-DKI_BITVOTING` (canonical) |
| `otto-score-ifc/lib/ki-train.h` | Shared training loop (`ki_batch_correct`) |
| `otto-score-ifc/lib/ki-load.h` | Shared data loading |
| `out/archived-trn/mlp-bin32-otto-trn-bitvoting.c` | Original standalone tool (archived 2026-08-02) |
