 ...
 *
 * Vendored from https://github.com/wizzard/libtprint (master, 2026-08-12)
 * — unmodified except a (gint) cast to satisfy -Wconversion.
 * Licensed under GPL-3.0 (header below). Requires glib-2.0 (pkg-config).
 * Used for the --debug-class-voting table output in the Otto trainers.
 *
...
void tprint_column_add (TPrint *tprint, const gchar *caption, TPrintAlign caption_align, TPrintAlign data_align)
{
    ...
    if (tprint->show_header) {
        ...
        col->max_width = (gint)strlen (caption);  /* -Wconversion fix */
    } else {
        col->caption = NULL;
        col->max_width = 0;
    }
