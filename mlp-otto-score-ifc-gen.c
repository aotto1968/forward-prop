/*
 * ki-w2/mlp-otto-score-ensemble.c — Otto Score Ensemble Trainer
 * =============================================================
 *
...
#define OTTO_VERSION 5U            /* v5 = ensemble */
...
static void export_ensemble(const char *out_dir,
    ...
    uint32_t magic = OTTO_MAGIC, ver = OTTO_VERSION, mode = H0_MODE_VAL;
    ...
    size_t w0_bytes = (size_t)H * NC * 4;
    size_t tgt_bytes = (size_t)H * 10 * 32 * 4;
    size_t off_bytes = 10 * 8;
    size_t total = 0;

    for (int m = 0; m < ensembleN; m++) {
        fwrite(W0_ens + (size_t)m * H * NC, 4, (size_t)H * NC, f);
        fwrite(target_ens + (size_t)m * 10 * H * 32, 4, (size_t)H * 10 * 32, f);
    ...
    printf("\n══╡ EXPORT ╞═══════════════════════════════════════════════════\n");
    printf("  Model:  %s  (v%u, %d ensemble, F=%d)\n",
           path, OTTO_VERSION, ensembleN, 1<<OT_PRECISION);
...
static void print_setup(int H, int epochs, int trainN, int evalN,
    ...
    printf("  W0:          random uint32[%d][%d]  (frozen, %zu KB)\n",
           H, NC, (size_t)H * NC * sizeof(uint32_t) / 1024);
    ...
    printf("  ───────────────────────────────────────────────────────────\n");
    printf("  Precision:   OT_PRECISION=%d  (F=%d, step=%d, lr=%.4f)\n",
           OT_PRECISION, 1<<OT_PRECISION,
           (int)(0.05 * (1<<OT_PRECISION) + 0.5), 0.05);
    printf("══════════════════════════════════════════════════════════════════════\n");
}
...
int main(int argc, char *argv[]) {
    ...
    size_t w0_sz = (size_t)ensembleN * (size_t)H * NC;
    ...
    if (a.random_file[0]) {
        ...
        for (int m = 0; m < ensembleN; m++) {
            w0_srandom((unsigned int)(a.seed + m));
            for (size_t i = 0; i < (size_t)H * NC; i++)
                W0_ens[(size_t)m * H * NC + i] = w0_random();
        }
    } else if (a.ensemble_seed[0] && strcmp(a.ensemble_seed, "const") == 0) {
        /* const: same seed for every member → identical W0 */
        w0_srandom((unsigned int)a.seed);
        size_t row_sz = (size_t)H * NC;
    ...
    for (int m = 0; m < ensembleN; m++) {
        ...
        const uint32_t *W0_m = W0_ens + (size_t)m * H * NC;
     ...
     */
    int step = (a.lr > 0) ? (int)ot_precision(a.lr) : a.lr_step;
    ...
    for (int ep = 0; ep < epochs; ep++) {
        ...
        printf("Ep %2d  trn=%.1f%%  evl=%.1f%%  best=%.1f%%  step=%d  time=%dms",
               ep + 1, trn_acc, evl_acc, best_evl, step, elapsed);
        ...
        /* Correction pass for each ensemble member */
        for (int m = 0; m < ensembleN; m++) {
            int corrections = 0;
            #pragma omp parallel for reduction(+:corrections) schedule(static)
            for (int s = 0; s < total_train; s++) {
                int true_k = (int)y_tr[s];
                int64_t scores[10];
                scores_otto(X_tr + (size_t)s * NC,
                           W0_ens + (size_t)m * H * NC, H,
                           target_ens + (size_t)m * tgt_sz_m,
                           offset_ens + (size_t)m * 10, scores);
                int pred = 0;
                for (int k = 1; k < 10; k++)
                    if (scores[k] > scores[pred]) pred = k;
                ...
                if (pred != true_k) {
                    ...
                    for (int h = 0; h < H; h++) {
                        uint32_t h0 = h0_neuron(in, W0_ens + (size_t)m * H * NC + (size_t)h * NC);
                        for (int b = 0; b < 32; b++) {
                            if (h0 & (1U << (unsigned)b)) {
                                #pragma omp atomic
                                target_ens[(size_t)m * tgt_sz_m + TGT_IDX(true_k, h, b, H)] += step;
                                #pragma omp atomic
                                target_ens[(size_t)m * tgt_sz_m + TGT_IDX(pred, h, b, H)] -= step;
                            }
                        }
                    }
                    corrections++;
    ...
    printf("  ENSEMBLE H=%d  N=%d  ep=%d  trn=%.1f%%  evl=%.1f%%  best=%.1f%%  lr=%.4f  time=%dms\n",
           ...
           (double)step / (1<<OT_PRECISION), elapsed_ms);
