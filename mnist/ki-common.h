 ...
 *
 * Alle encodings erzeugen eine Bitmaske: popcount(encode(pv)) ∝ Helligkeit.
 * XNOR+popcount zählt dann Schwellen-Übereinstimmungen.
 */
enum ki_encoding {
    KI_ENC_RAW  = 0,
    KI_ENC_LIN7 = 1,
...
/* ── Encoding-Namen (kurz, ohne Width — für Display mit Suffix) ── */
__attribute__((unused))
static const char *ki_enc_name_short(int enc) {
...
/* ── Parser: String → Encoding-Index + Width-Suffix ──────────
 ...
 * Stores parsed width in *out_width. */
#define KI_ENC_WIDTH_DEFAULT 8
__attribute__((unused))
static inline int ki_enc_parse(const char *tok, int *out_width) {
    /* Versuche bekannte Width-Suffixe: 32, 16, 8 (längster zuerst)
     * "exp16" → prefix="exp", width=16
     * "lin816"→ prefix="lin8", width=16
     * "lin8"  → prefix="lin8", width=8 (8 ist auch Suffix)
     * "lin"   → prefix="lin", width=default */
    ...
    for (int wi = 0; wi < 3; wi++) {
        ...
        if (tok_len > wslen && strcasecmp(tok + tok_len - wslen, wstr) == 0) {
            ...
            prefix[plen] = '\0';
            /* Versuche Prefix als Encoding-Name */
    ...
    /* Kein Width-Suffix: exakter Match */
...
/* ── Encoding anwenden (per Pixel) ─────────────────────────────
 * Transformiert einen 8-Bit-Pixelwert pv (0..255) in eine
 * Thermometer-Bitmaske. Popcount(Ergebnis) ∝ Helligkeit.
 * Alle Varianten arbeiten ganzzahlig — keine Floats zur Laufzeit.
 * LOG/EXP/SIG sind Float-Varianten für Host-Training.
 */
__attribute__((unused))
static inline uint32_t ki_apply_enc_w(uint8_t pv, int enc, int width) {
    /* width = encoding output width in bits (8, 16, 32).
     * Bestimmt max_lev (=width), NICHT gecappt.
     * Jedes Encoding hat width-spezifische Parameter. */
    int max_lev;
    switch (enc) {
      case KI_ENC_LIN7: max_lev = 7; break;       /* fest 7 Level */
      default:          max_lev = width; break;    /* skaliert mit Breite */
    }
    ...
    case KI_ENC_EXP: {
        /* Mapping: pv (0..255) → level (0..width) via Exponentialkurve.
         * Level = Anzahl 1-Bits im Output = Thermometer-Stufe.
         *   width=8:  exp=0.30 (MNIST-optimiert, 97.1%)
         *   width=16: exp=0.35 (16 Stufen, 97.5%)
         *   width=32: exp=0.30 (32 Stufen)
         * max_lev = width (ausser LIN7: fix 7) — siehe switch oben. */
    ...
    case KI_ENC_SIG: {
        ...
        // optimized for: --hiddenN 128 --ensembleN 5 --epochsN 5 --encoding by=sig,rg=sig --lr 0.1 --step-power 5
        // float s = 1.0f / (1.0f + expf(-18.0f * (norm - 0.5f)));
...
/* ── 8-Bit Kurzform (für legacy-Aufrufe) ────────────────────────── */
__attribute__((unused))
...
/* ═══════════════════════════════════════════════════════════════════════
 * COLOR DEFINITIONS — zentrale Bit-Positionen + Parser
 * ═══════════════════════════════════════════════════════════════════════
 *
 * Jeder Analyse-Block hat eine feste Bit-Position (0..COLOR_NB-1).
 * Der --channels Parser nutzt diese Enum-Werte.
 * Die Default-Maske pro Dataset:
 *   CIFAR: r+g+b = (1<<COLOR_R)|(1<<COLOR_G)|(1<<COLOR_B) = 0x0E
 *   MNIST: mnist = (1<<COLOR_MNIST) = 1
 *
 * Ein Enum statt Define-Gruppe: type-sicher, debugger-freundlich.
 *
 * Bit 0 (COLOR_MNIST) ist immer der einzelne Graustufen-Block für MNIST.
 * Alle anderen Blöcke (R,G,B,Y,…) sind um 1 verschoben.
 */
enum ki_color_bit {
    COLOR_MNIST = 0,  /* single grayscale block (MNIST), immer 1 */
    COLOR_R     = 1,  /* raw R */
    COLOR_G     = 2,  /* raw G */
    COLOR_B     = 3,  /* raw B */
...
/* Dataset-spezifische Defaults (in ki-local.h definiert):
 *   KI_DEFAULT_COLOR — Bitmaske für die Standard-Kanäle
 *   z.B. MNIST → (1<<COLOR_Y), CIFAR → (1<<COLOR_R)|(1<<COLOR_G)|(1<<COLOR_B)
 */
